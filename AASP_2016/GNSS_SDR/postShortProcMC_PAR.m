% Script postShortProcMC_PAR.m processes the raw signal from the specified data
% file (in settings) operating on blocks of 11 seconds of data.
%
...
%% Acquisition ============================================================
    ...
    %if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        %acqResults = acquisitionMC(data, settings);
...
%% Initialize channels and prepare for the run ============================
    ...
    if (any(acqResults.carrFreq))
        ...
        channel = orderStruct(channel);
        %if (choose == 1)
        %    showChannelStatus(channel, settings);
        %else
        %    showChannelStatus(satellites,channel,settings);
        %end %if (choose == 1)
    else
...
%% Track the signal =======================================================
    if(settings.skipTracking == 0)
        ...
        if settings.nrMsProcess > 36000 
            [trackResults, channel] = tracking(fid, channel, settings);
        else
            [trackResults, channel] = trackingShort(fid, channel, settings);
        end %if settings.nrMsProcess > 36000 
